...
from NLP_Pipeline.Servers.helpers import get_token

from NLP_Pipeline.Servers.Storage.FileSystemStorage import FileSystemStorage
from NLP_Pipeline.Tools.toolsInterface import known_tools
from NLP_Pipeline.Workers.worker import run_workers

from NLP_Pipeline.Servers.ServerTypes.RESTServer import app_restServer

app = Flask('NLPipe', template_folder=os.path.dirname(__file__))  # creating the Flask app
...
if __name__ == '__main__':  # main thread starting, mostly reading the arguments
    ...
    parser.add_argument("directory", nargs="?",
                        help="Location of NLPipe storage directory (default: $NLP-PIPE_DIR or tempdir)")
    parser.add_argument("--workers", "-w", nargs="*", help="Run specified or all known worker modules")
    parser.add_argument("--port", "-p", type=int, default=5001,
                        help="Port number to listen to (default: $NLP-PIPE_PORT or 5001)")
    parser.add_argument("--host", "-H", help="Host address to listen on (default: $NLP-PIPE_HOST or localhost)")
    ...
    host = args.host or os.environ.get("NLP-PIPE_HOST", "localhost")
    port = args.port or os.environ.get("NLP-PIPE_PORT", 5001)
    ...
    if not args.directory:  # directory for saving documents
        if "NLP-PIPE_DIR" in os.environ:
            args.directory = os.environ["NLP-PIPE_DIR"]
        else:
            tempdir = tempfile.TemporaryDirectory(prefix="NLP-PIPE_")
